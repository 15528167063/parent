...
public class TotleAppFragment extends BaseFragment implements View.OnClickListener {
    ...
    public static Fragment getInstance(int classesId) {
        TotleAppFragment fragment = new TotleAppFragment();
    ...
    public void iniData() {
        ...
        if (mlist.size() == 0) {//没有app数据
            ...
            sp.edit().putBoolean("isappuse",false).commit();    //用于标识区别  应用使用列表和全部列表
            sp.edit().putBoolean("isapplist",true).commit();    //主动申请才修改appp
            EventBus.getDefault().post(new RequestChildAppListEvent(childUUID, parentUUID), "requestChangechildApp");
            hd.postDelayed(runnable, 8000);
        }
        ...
        if (list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                ...
                if (map.get(i)) {
                    ...
                    String apppackgename = appInfo.getApppackgename();
                    AppDataBean findApp = dao.queryBuilder().where(AppDataBeanDao.Properties.Apppackgename.eq(apppackgename)).where(AppDataBeanDao.Properties.Childuuid.eq(childUUID)).build().unique();
                    if (findApp != null) {
                        if(blackclick){
                            findApp.setAppwhitelist(CmdCommon.CMD_FLAG_BLACK);//1代表白名单
                        }else {
                            findApp.setAppwhitelist(CmdCommon.CMD_FLAG_Te);//1代表白名单
                        }
                        dao.update(findApp);
