            ...
            if (list.size() > 0) {
                for (int i = 0; i < list.size(); i++) {
                    if (map.get(i)) {
                        ...
                        AppDataBean findApp = dao.queryBuilder().where(AppDataBeanDao.Properties.Apppackgename.eq(apppackname)).where(AppDataBeanDao.Properties.Childuuid.eq(childUUID)).build().unique();
                        if (findApp != null) {
                            if(addwhite){
