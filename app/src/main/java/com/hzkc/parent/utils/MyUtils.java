...
public class MyUtils {
    ...
    public static List<String> spliteData(String data) {
        List<String> dataList = new ArrayList<String>();
        if(false){
            int len = data.length();
            int count = len / CmdCommon.DATA_SPLITE_BYTES;
            int remain = len % CmdCommon.DATA_SPLITE_BYTES;

            for (int i = 0; i < count; i++) {
                String tempData = data.substring(i * CmdCommon.DATA_SPLITE_BYTES, (i * CmdCommon.DATA_SPLITE_BYTES) + CmdCommon.DATA_SPLITE_BYTES);
                dataList.add(tempData);
            }
            if (remain > 0) {
                dataList.add(data.substring(len - remain));
            }
            for (int i = 0; i < dataList.size(); i++) {
                LogUtil.e("fuck", i + " " + dataList.get(i));
            }
        }else{
            dataList.add(data);
        }
...
//            e.printStackTrace();
    ...
    public static Boolean isNeedUpdate(Context context,String currentVersion, String serverVersion) {
        LogUtil.e("isNeedUpdate", "isNeedUpdate");
        ...
        if (curS == null || servS == null) {
            LogUtil.e("isNeedUpdate", "null fa sheng l");
            return false;
        }
        if (curS.length < 3 || servS.length < 3) {
            LogUtil.e("isNeedUpdate", "<3 fuck");
            return false;
        }
        ...
        LogUtil.e("isNeedUpdate", "" + cur1 + " " + cur2 + " " + cur3 + " " + serv1 + " " + serv2 + " " + serv3);
        int cur4 = 0;
        int serv4 = 0;
        if (curS.length > 3) {
