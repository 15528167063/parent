...
public class AddTeamInternetPlanActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initDialogView(final TextView tv) {
        ...
        //初始化小时
        wvTimeHour.setWheelAdapter(new ArrayWheelAdapter(AddTeamInternetPlanActivity.this)); // 文本数据源
        ...
        //初始化分钟
        wvTimeMinute.setWheelAdapter(new ArrayWheelAdapter(AddTeamInternetPlanActivity.this)); // 文本数据源
    ...
    private void saveStopPlan() {
        if(TextUtils.isEmpty(etPlanName.getText().toString())){
            Toast.makeText(this, "请输入计划标签名", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        String weekDay= "";
        if(planlist!=null && planlist.size()>=20){
        ...
        NetPlanData netPlanData=new NetPlanData(planName,weekDay, startPlanTime,endPlanTime,"1");
        planlist.add(netPlanData);
