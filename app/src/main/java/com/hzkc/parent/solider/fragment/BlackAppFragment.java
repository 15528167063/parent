...
public class BlackAppFragment extends BaseFragment implements View.OnClickListener {
    ...
    public static Fragment getInstance(int classesId) {
        BlackAppFragment fragment = new BlackAppFragment();
    ...
    @Override
    public void iniData() {//冷  桌    137 14999     华山70 练 117   99999     沧海70   394  14999
            ...
            if (list.size() > 0) {
                for (int i = 0; i < list.size(); i++) {
                    if (map.get(i)) {
                        ...
                        List<AppDataBean> list = dao.queryBuilder().where(AppDataBeanDao.Properties.Appname.eq(appName)).where(AppDataBeanDao.Properties.Childuuid.eq(childUUID)).build().list();
                        for (int j = 0; j < list.size(); j++) {
                            Log.e("------------"+j,new Gson().toJson(list.get(j)));
                        }
                        ...
                        if (findApp != null) {
                            ...
                            dao.update(findApp);
                            ((AppManageSoliderActivity)getActivity()).addDatas(findApp);
                        }
                    }
                }
                list.clear();
