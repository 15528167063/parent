...
public class AddStopInternetPlanActivity extends BaseActivity implements View.OnClickListener {
    private LinearLayout activityAddStopInternetPlan;
    ...
    private static final String TAG = "AddStopInternetPlanActi";
    ...
    private void initDialogView(final TextView tv) {
        ...
        //初始化小时
        wvTimeHour.setWheelAdapter(new ArrayWheelAdapter(AddStopInternetPlanActivity.this)); // 文本数据源
        wvTimeHour.setWheelSize(3);
        wvTimeHour.setLoop(true);
        WheelView.WheelViewStyle style1 = new WheelView.WheelViewStyle();
        style1.selectedTextSize = 20;
        style1.textSize = 20;
        ...
        //初始化分钟
        wvTimeMinute.setWheelAdapter(new ArrayWheelAdapter(AddStopInternetPlanActivity.this)); // 文本数据源
        wvTimeMinute.setWheelSize(3);
        wvTimeMinute.setLoop(true);
        WheelView.WheelViewStyle style2 = new WheelView.WheelViewStyle();
        style2.selectedTextSize = 20;
        style2.textSize = 20;
        ...
        LogUtil.i(TAG, tv.getText().toString().substring(3)+"initDialogView: "+minuteList.indexOf(tv.getText().toString().substring(3)));
    ...
    private void initView() {
        activityAddStopInternetPlan = (LinearLayout) findViewById(R.id.activity_add_stop_internet_plan);
...
//                break;
            case R.id.bt_comfirm:
                boolean sptishi =sp.getBoolean("gkjhtishi",false);
                if(!sptishi){
                    showTiShiDialog();
                    return;
                }
    ...
    private void saveStopPlan() {
        ...
        if(list!=null &&  list.size()>=20){
            Toast.makeText(this, "添加数目不能超过20条", Toast.LENGTH_SHORT).show();
            return;
        }
        if(yiFlag){
            weekDay=weekDay+"1";
...
//        List<NetPlanDataBean> list = planDao.queryBuilder().where(NetPlanDataBeanDao.Properties.Childuuid.eq(childuuid)).build().list();
        ...
        NetPlanDataBean planInfo= new NetPlanDataBean(null,childuuid,planName,weekDay,
                startPlanTime,endPlanTime,"1");
        planDao.insert(planInfo);
    ...
    public void showTiShiDialog(){
        ...
        tvFinish1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                showDialog.dismiss();
                if(cb_box.isChecked()){
                    sp.edit().putBoolean("gkjhtishi",true).commit();
                }
                saveStopPlan();
            }
        });
